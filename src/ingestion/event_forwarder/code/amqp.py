...
class MQConnection():
    ...
    def __init__(self, conf, conf_section='broker'):
        ...
        self.connection = None # aiormq.Connection
        ...
        self.connection_name = conf.get(conf_section, 'connection_name', fallback='event-shipper')
    ...
    async def consume(self, on_message):
        ...
        if self.qos:
            LOG.info('QOS to %s', self.qos)
            await self.consumer.basic_qos(prefetch_count=self.qos)

        LOG.info('Consuming from <%s>', self.queue)
        return await self.consumer.basic_consume(self.queue, on_message,
                                                 consumer_tag=self.connection_name)
