...
CREATE OR REPLACE FUNCTION fs.file2ino(_name text)
RETURNS bigint
LANGUAGE plpgsql
AS $_$
BEGIN
...
RETURNS TABLE(ino bigint, display_name text, ctime bigint, mtime bigint, nlink int4)
LANGUAGE plpgsql AS $_$
BEGIN
...
CREATE OR REPLACE FUNCTION fs.lookup_file(_userid            bigint,
			       		  ...
			       		  _filename          varchar)
RETURNS TABLE(ino bigint, stable_id text, num_datasets int4, display_name text, ctime bigint, mtime bigint,
	      header_size integer, payload_size bigint)
LANGUAGE plpgsql AS $_$
DECLARE
...
BEGIN
    ...
    RETURN QUERY 
	SELECT --DISTINCT 
	       fs.file2ino(ft.stable_id) AS ino,
	       ...
	       fs.header_size(pft.header_size, number_recipient_keys) AS header_size,
	       pft.payload_size AS payload_size
	FROM public.dataset_table dt
	...
	WHERE dt.stable_id = _dataset_stable_id
	      AND CASE
		       ...
		       THEN ft.display_name = _filename
                       ELSE TRUE
		  END
	;
END;
...
CREATE OR REPLACE FUNCTION fs.get_file_info(_username varchar,
                                            _file_ino bigint)
RETURNS TABLE(mount_point text, relative_path text, header bytea, payload_size bigint)
LANGUAGE plpgsql AS $_$
DECLARE
...
BEGIN
    ...
    _file_stable_id := fs.ino2str(_file_ino);
    ...
    RETURN QUERY
    SELECT current_setting('vault.dirpath') AS mount_point,
           pft.relative_path AS relative_path,
...
CREATE OR REPLACE FUNCTION fs.readdir(_username  varchar,
       			              _parent_ino bigint,
                                      _offset    bigint,
			   	      _limit     bigint, --DEFAULT 1000000
				      _as_c4gh   boolean DEFAULT TRUE)
RETURNS TABLE(ino bigint, display_name text, ctime bigint, mtime bigint, nlink int4,
	      size bigint, is_dir boolean)
LANGUAGE plpgsql AS $_$
DECLARE
	user_number_id        bigint;
	dataset           varchar;
BEGIN
    ...
    IF _parent_ino = 1 THEN
        -- list datasets
	RETURN QUERY
	SELECT q.ino, q.display_name, q.ctime, q.mtime, q.nlink,
	       1::bigint AS size,
	       TRUE AS is_dir
	...
	SELECT q.ino, 
	       CASE WHEN _as_c4gh
	            THEN q.display_name || '.c4gh'
		    ELSE q.display_name
	       END AS display_name,
	       q.ctime, q.mtime, q.nlink,
	       CASE WHEN _as_c4gh
	            THEN q.header_size::bigint + q.payload_size
		    ELSE fs.decrypted_filesize(q.payload_size)
	       END AS size,
	       FALSE AS is_dir
        FROM fs.get_files(user_number_id, fs.ino2str(_parent_ino), _offset, _limit) q;
    END IF;
...
CREATE OR REPLACE FUNCTION fs.lookup(_username varchar,
			             ...
			             _name varchar,
				     _as_c4gh   boolean DEFAULT TRUE)
RETURNS TABLE(ino bigint, ctime bigint, mtime bigint, nlink int4, size bigint, is_dir boolean)
LANGUAGE plpgsql AS $_$
DECLARE
	user_number_id        bigint;
BEGIN
    ...
    ELSE
        ...
        IF _as_c4gh THEN
	   _name := substring(_name from '(.*)\.c4gh$'); -- remove .c4gh
       	   IF _name IS NULL THEN
              RAISE EXCEPTION 'invalid filename or not a C4GH file';
           END IF;
	   RAISE NOTICE 'extracted filename: %', _name;
	END IF;
	...
	FROM fs.lookup_file(user_number_id, fs.ino2str(_parent_ino), _name) q
	LIMIT 1;
    END IF;
...
CREATE OR REPLACE FUNCTION fs.getattr(_username varchar,
       			              _ino bigint,
				      _as_c4gh   boolean DEFAULT TRUE)
RETURNS TABLE(ctime bigint, mtime bigint, nlink int4, size bigint, is_dir boolean)
LANGUAGE plpgsql AS $_$
DECLARE
	user_number_id        bigint;
	number_recipient_keys int;
...
BEGIN
    ...
    IF _ino >= 200000000000 AND _ino < 400000000000 THEN
        -- a file
	-- get number of recipients
	...
	SELECT extract(epoch from pft.created_at)::bigint  AS ctime,
	       ...
	       CASE WHEN _as_c4gh
	            THEN fs.header_size(pft.header_size, number_recipient_keys)::bigint + pft.payload_size
		    ELSE fs.decrypted_filesize(pft.payload_size)
	       END AS size,
	       FALSE::boolean AS is_dir
...
CREATE OR REPLACE FUNCTION fs.listxattr(_username varchar, _ino bigint)
RETURNS bytea
LANGUAGE plpgsql AS $_$
DECLARE
	user_number_id        bigint;
BEGIN
    ...
    IF _ino < 200000000000 OR _ino > 400000000000 THEN
        RETURN NULL; --RAISE EXCEPTION 'Not a file: %', _ino;
    END IF;

    RETURN convert_to('user.accession_id'      , 'UTF8') || '\x00'::bytea ||
           convert_to('user.decrypted_filesize', 'UTF8') || '\x00'::bytea;
...
BEGIN
    ...
    ELSEIF _name = 'user.accession_id' THEN

        _stable_id := fs.ino2str(_ino);
        -- RAISE NOTICE 'looking up file %', _stable_id;
	-- Check if it exists. Shouldn't be asked, cuz a lookup on the file system wouldn't find it
        SELECT ft.stable_id INTO _res
	FROM public.file_table ft
	WHERE ft.stable_id = _stable_id
	LIMIT 1;
    END IF;
